...
const parseColor = (colorString: string): { name: string; hex: string } => {
  // Если цвет содержит #, значит это новый формат "название#hex"
  if (colorString.includes('#')) {
    const [name, hex] = colorString.split('#');
    const capitalizedName = name.trim().charAt(0).toUpperCase() + name.trim().slice(1);
    return { name: capitalizedName, hex: `#${hex}` };
  }
  
  // Если это старый формат (только hex), возвращаем hex как есть и пустое название
...
export const HoroscopeSection = () => {
  ...
  const predictionTextStyle: React.CSSProperties = {
    fontWeight: 300,
    lineHeight: isVerySmallMobile ? '18px' : isMobile ? '19px' : '21px',
    fontSize: isVerySmallMobile ? '13px' : isMobile ? '14px' : '16px',
    marginBottom: isVerySmallMobile ? '8px' : isMobile ? '10px' : '16px',
    textAlign: 'justify'
  };
  ...
  const infoRowStyle: React.CSSProperties = {
    display: 'flex',
    alignItems: 'center',
    gap: isVerySmallMobile ? '4px' : isMobile ? '5px' : '8px',
    flexWrap: 'wrap',
    marginBottom: isVerySmallMobile ? '4px' : isMobile ? '6px' : '12px'
  };
  ...
  const labelStyle: React.CSSProperties = {
    fontWeight: 'normal',
    fontSize: isVerySmallMobile ? '13px' : isMobile ? '14px' : undefined
  };
  ...
  const moodValueStyle: React.CSSProperties = {
    fontSize: isVerySmallMobile ? '18px' : isMobile ? '19px' : '20px'
  };

  const numberIconStyle: React.CSSProperties = {
    width: isVerySmallMobile ? '16px' : isMobile ? '20px' : '24px',
    height: isVerySmallMobile ? '16px' : isMobile ? '20px' : '24px',
    objectFit: 'contain',
    flexShrink: 0
  };

  const numberValueStyle: React.CSSProperties = {
    fontSize: isVerySmallMobile ? '15px' : isMobile ? '16px' : '16px'
  };
      ...
      <div style={contentWrapperStyle}>
        <div style={mainContentStyle}>
          ...
          <div style={imageContainerStyle}>
            ...
            {horoscope && (
              <div style={{
                ...
                borderRadius: '12px',
                background: isMobile 
                  ? `radial-gradient(circle at center, ${getDayColor()}20 0%, ${getDayColor()}10 20%, transparent 50%)`
                  : `radial-gradient(circle at center, ${getDayColor()}25 0%, ${getDayColor()}15 30%, transparent 70%)`,
                pointerEvents: 'none',
                zIndex: 5,
            ...
            {!loading && horoscope && (
              ...
              >
                ...
                <div style={colorNameStyle}>
                  {parseColor(horoscope.color).name || 'Цвет дня'}
                </div>
          ...
          <div style={textContentStyle}>
                ...
                <div style={infoContainerStyle}>
                  ...
                  <div style={infoRowStyle}>
                    ...
                    <div style={valueContainerStyle}>
                      ...
                      />
                      <div style={numberValueStyle}>{horoscope.number}</div>
                    </div>
                  </div>
                  ...
                  <div style={infoRowStyle}>
                    ...
                    <div style={valueContainerStyle}>
                      ...
                      <div style={colorValueStyle}>{parseColor(horoscope.color).name || 'Цвет дня'}</div>
                    </div>
                  </div>
                </div>
            ...
            {/* Decorative Elements - всегда видимы внизу контейнера */}
            <div style={{
              marginTop: 'auto',
              paddingTop: '16px'
            }}>
